    ...
    <div className="min-h-screen bg-gray-950">
      ...
      <main className="max-w-2xl mx-auto px-4 py-8 space-y-6">
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between p-3 glass rounded-xl">
              ...
              <button
                ...
                className={clsx('w-12 h-6 rounded-full transition-all duration-200 relative', profile.isPublic ? 'bg-brand-500' : 'bg-gray-700')}
              >
            ...
            {profile.isPublic && (
              <div className="flex items-center gap-2 text-sm text-gray-400">
                ...
                Public link: <a href={`/u/${user?.username}`} className="text-brand-400 hover:underline" target="_blank">/u/{user?.username}</a>
              </div>
            )}
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between p-3 glass rounded-xl">
              ...
              <button
                ...
                className={clsx('w-12 h-6 rounded-full transition-all duration-200 relative', profile.notificationsEnabled ? 'bg-brand-500' : 'bg-gray-700')}
              >
            ...
            {profile.notificationsEnabled && (
              <div className="space-y-3">
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1.5">Digest Time</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1.5">Reminder Email</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            {ALL_BADGES.map((badge) => {
              ...
              return (
                <div
                  key={badge.id}
                  className={clsx('p-3 rounded-xl border transition-all', earned ? 'bg-brand-500/10 border-brand-500/30' : 'bg-gray-900/50 border-gray-800 opacity-50 grayscale')}
                >
                  <div className="text-lg mb-1">{badge.name.split(' ')[0]}</div>
                  <div className="text-xs font-semibold text-white">{badge.name.split(' ').slice(1).join(' ')}</div>
          ...
          <Section title="Archived Habits" icon={Archive}>
            <div className="space-y-2">
              {archivedHabits.map((habit) => (
                <div key={habit._id} className="flex items-center justify-between p-3 glass rounded-xl">
        ...
        <Section title="Export Data" icon={Download}>
          <div className="space-y-3">
            <p className="text-sm text-gray-400">Download all your habit completion data</p>
            <button
